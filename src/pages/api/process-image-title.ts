  ...
  return new Promise((resolve, reject) => {
    ...
    busboy.on("file", (name, file, info) => {
      ...
      file.on("end", () => {
        ...
        console.log(`File processed: ${fileBuffer.length} bytes`);
      });
    });
...
export default async function handler(
  ...
  try {
    ...
    console.log("Image preprocessing complete, starting OCR...");

    // Try Engine 2 first
    let result = await processOCR(imageBuffer, language, "2");
    console.log("Engine 2 response received");
    ...
    if (
      ...
      console.warn("Engine 2 failed or busy. Retrying Engine 1...");
      await new Promise((r) => setTimeout(r, 1000));
      result = await processOCR(imageBuffer, language, "1");
      console.log("Engine 1 response received");
    }

    if (result?.IsErroredOnProcessing) {
      console.error("OCR processing error:", result.ErrorMessage?.[0]);
      return res.status(500).json({
        status: "error",
        message: result?.ErrorMessage?.[0] || "OCR processing error",
      });
