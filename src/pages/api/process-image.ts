...
//   }
// }
// pages/api/process-image.ts (Alternative version)
...
// ... rest of your existing functions (processOCR, cleanSearchQueries)
...
export default async function handler(
  ...
  try {
    ...
    const imageBuffer = await sharp(fileBuffer)
      ...
      .grayscale()
      .modulate({ brightness: 1, saturation: 2 })
      .png()
      .toBuffer();

    // ... rest of your OCR processing logic
  } catch (error: unknown) {
    console.error("OCR API Error:", error);
