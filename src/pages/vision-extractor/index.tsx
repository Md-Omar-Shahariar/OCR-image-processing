...
function VisionExtractor() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    for (const file of files) {
      ...
      try {
        const res = await fetch("/api/process-image-vision", {
          method: "POST",
          body: formData,
        });
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-6xl mx-auto px-4 pt-8 pb-4">
          <PageHeaderCard
            ...
            subtitle="Powered by Google Cloud Vision OCR"
            stats={[
              { label: "Engine", value: "Vision API (TEXT_DETECTION)" },
              { label: "Average run", value: "~3-5s per image" },
            ]}
          />
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border border-white/50 overflow-hidden mb-8">
            {/* Header Section */}
            <div className="bg-gradient-to-r from-sky-600 via-blue-600 to-emerald-500 p-8 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-32 translate-x-32"></div>
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-white/10 rounded-full translate-y-24 -translate-x-24"></div>

              <div className="relative z-10">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 leading-tight">
                  Extract Titles & Links with Google Vision
                </h1>
                <p className="text-blue-50 text-base sm:text-lg max-w-2xl">
                  Upload SERP screenshots and let Google Cloud Vision return
                  structured titles, URLs, and snippets with multilingual
                  accuracy.
                </p>
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                  {[
                    { label: "Ideal for", value: "SERP + marketplace screenshots" },
                    { label: "Accuracy boost", value: "Google Vision OCR" },
                    { label: "Privacy", value: "Processed in-session only" },
                  ].map((stat) => (
                    <div
                      key={stat.label}
                ...
                <div>
                  <div className="mb-8">
                    ...
                    <p className="text-slate-600">
                      ...
                      browse. Google Vision will detect and extract titles with
                      their corresponding URLs and descriptions.
                    </p>
                  </div>
                  ...
                  <UploadDropzone
                    ...
                    helperText="Optimized for SERP screenshots (Google Vision)"
                    maxSizeCopy="Supports JPG, PNG, BMP • Maximum 4MB per file"
          ...
          {!processing && results.length === 0 && files.length === 0 && (
            <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border border-white/50 p-8 sm:p-12 text-center">
              ...
              <h3 className="text-2xl font-bold text-slate-800 mb-3">
                Ready to Extract Titles with Vision?
              </h3>
              <p className="text-slate-600 text-lg mb-8 max-w-md mx-auto">
                Upload search result screenshots to automatically extract titles
                with their corresponding URLs and descriptions using Google
                Cloud Vision.
              </p>
...
export default withAuth(VisionExtractor);
